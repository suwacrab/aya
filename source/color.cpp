...
namespace aya {
	void CColor::write_alpha(scl::blob& out_blob) const {
		out_blob.write_u8(a);
	}
	void CColor::write_argb4(scl::blob& out_blob) const {
	...
	void CColor::write_argb8(scl::blob& out_blob) const {
		out_blob.write_u8(b);
		out_blob.write_u8(g);
		out_blob.write_u8(r);
		out_blob.write_u8(a);
	}
	void CColor::write_rgb565(scl::blob& out_blob) const {
	...
	void CColor::write_rgb5a1_sat(scl::blob& out_blob,bool msb) const {
		const auto short_r = ((uint32_t)r)>>3;
